...
exports.updateJob = async (req, res, next) => {
    try {
        ...
        const { judul_posisi, kategori, tipe_pekerjaan, gaji, deskripsi_pekerjaan } = req.body;
        await pool.query(
            'UPDATE lowongan SET judul_posisi = ?, kategori = ?, tipe_pekerjaan = ?, gaji = ?, deskripsi_pekerjaan = ? WHERE id_lowongan = ? AND id_perusahaan = ?',
            [judul_posisi, kategori, tipe_pekerjaan, gaji, deskripsi_pekerjaan, id, req.user.id_user]
        );
        res.json({ status: 'success', message: 'Lowongan berhasil diperbarui' });
    } catch (err) {
...
exports.getCompanyProfile = async (req, res, next) => {
    try {
        const [rows] = await pool.query(
            'SELECT * FROM profil_perusahaan WHERE id_user = ?',
            [req.user.id_user]
        );
        if (rows.length === 0) {
