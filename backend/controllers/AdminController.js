...
class AdminController {
  ...
  async getAllTestimonials(req, res) {
    try {
      const [rows] = await pool.query(
        `SELECT t.*, u.email as email_user 
         FROM testimonials t 
  ...
  async updateTestimonial(req, res) {
    try {
      const { id } = req.params;
      const { nama, role, perusahaan, teks, rating, is_active } = req.body;

      await pool.query(
        'UPDATE testimonials SET nama = ?, role = ?, perusahaan = ?, teks = ?, rating = ?, is_active = ? WHERE id_testimoni = ?',
        [nama, role || null, perusahaan || null, teks, rating || 5, is_active ?? 1, id]
      );
  ...
  async deleteTestimonial(req, res) {
    try {
      const { id } = req.params;
      await pool.query('DELETE FROM testimonials WHERE id_testimoni = ?', [id]);
